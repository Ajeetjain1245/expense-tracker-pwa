...
import { LayoutDashboard, Receipt, BarChart3, Plus, Wallet, LogOut, Download } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { NetworkStatusBadge } from './NetworkStatusBadge';

export const Navbar = ({ activeTab, setActiveTab, onOpenAddModal, onSyncComplete }) => {
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => setActiveTab('dashboard')}>
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">Offline PWA</p>
              </div>
            </div>
            ...
            {/* Primary Actions & Status Badges */}
            <div className="flex items-center gap-2 sm:gap-3">
              {/* Online/Offline & Sync Indicator */}
              <NetworkStatusBadge onSyncComplete={onSyncComplete} />

              {/* Install PWA button (shown if browser supports beforeinstallprompt) */}
              {deferredPrompt && (
                <button
                  onClick={handleInstallClick}
                  className="hidden sm:flex items-center gap-1.5 px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-semibold rounded-xl transition-all"
              ...
              <button
                ...
                className="flex items-center gap-2 px-3.5 sm:px-4 py-2 sm:py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs sm:text-sm font-semibold rounded-xl shadow-md shadow-emerald-600/20 active:scale-95 transition-all"
              >
